...
export const useModal = reactive({
  ...
  openModal(id, event = null) {
    ...
    if (event) {
      this.state.modalButton = event.currentTarget;
    }
  },
  state: {
    currentModal: null,
  ...
  setup(props, context) {
    ...
    const hidePageScroll = () => {
      if (modalStore.isModalOpen()) {
        ...
        documentElement.style.cssText = `
          position: fixed;
          top: -${modalStore.state.scrollPosition}px;
          right: 0;
          left: 0;
          margin-right: ${modalStore.state.scrollbarWidth}px;
          overflow-y: hidden;
        `;
      }
    };
    ...
    const showPageScroll = () => {
      if (!modalStore.isModalOpen()) {
        ...
        window.scrollTo(0, modalStore.state.scrollPosition);
      }
    };
